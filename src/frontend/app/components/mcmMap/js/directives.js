...
angular.module('mcmMapDirectives', ['Config', 'knalledgeMapServices'])
	...
	.directive('mcmMap', ['$injector', '$timeout', '$rootScope', '$routeParams', 'ConfigMap', '$compile', 'McmMapSchemaService', 'KnalledgeMapService', 'KnalledgeMapViewService', 'McmMapVisualService', 'McmMapVariableOperatorService', 'McmMapGridService', 'McmMapViewService',
		function($injector, $timeout, $rootScope, $routeParams, ConfigMap, $compile, McmMapSchemaService, KnalledgeMapService, KnalledgeMapViewService, McmMapVisualService, McmMapVariableOperatorService, McmMapGridService, McmMapViewService){
		...
		return {
			...
			controller: function ( $scope, $element) {
				...
				var mcmMapClientInterface = {
					...
					mapEntityClicked: function(mapEntity /*, mapEntityDom*/){
						$scope.$apply(function () {
							...
							GlobalEmitterServicesArray.get(eventName).broadcast('mcmMap', {mapEntity: mapEntity, emittingScope: $scope});
						});
					},
				...
				KnalledgeMapViewService.provider.config.nodes.showTypes = false;
				KnalledgeMapViewService.provider.config.edges.showNames = false;
				KnalledgeMapViewService.provider.config.edges.showTypes = false;
				...
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMap', function(eventModel) {
					// not the map type we are interested in
					if(eventModel.properties.type != 'mcm_map') return;

					console.log("[mcmMap.controller::GlobalEmitterServicesArray] ModelMap  nodes(len: %d): %s",
						eventModel.map.nodes.length, JSON.stringify(eventModel.map.nodes));
					console.log("[mcmMap.controller::GlobalEmitterServicesArray] ModelMap  edges(len: %d): %s",
				...
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMap', function(toolEntity) {
				...
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMap', function(mapEntity) {
					mcmMap.update();
				});

				eventName = "mapEntitySelectedEvent";
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMap', function(eventObj) {
				...
				var eventName = "mapStylingChangedEvent";
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMap', function() {
					console.log("[mcmMap.controller::GlobalEmitterServicesArray] event: %s", eventName);
					mcmMap.update();
				});
	...
	.directive('mcmMapTools', ['$injector', "$rootScope", "$timeout", 'ConfigMapToolset', 'McmMapSchemaService',
		function($injector, $rootScope, $timeout, ConfigMapToolset, McmMapSchemaService){
		var GlobalEmitterServicesArray = $injector.get('GlobalEmitterServicesArray');
			...
			controller: function ( $scope, $element) {
				...
				var toolsetClientInterface = {
					...
					toolEntityClicked: function(toolEntity){
						...
						GlobalEmitterServicesArray.get(eventName).broadcast('mcmMapTools', clickedToolEntity);
				...
				var eventName = "mapEntitySelectedEvent";
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMapTools', function(eventObj) {
					if(eventObj.mapEntity){
						console.log("[mcmMapTools.controller::GlobalEmitterServicesArray] ModelMap  mapEntity (%s): %s", eventObj.mapEntity.kNode.type, eventObj.mapEntity.kNode.name);
					}
					$scope.tools.length = 0;
	...
	.directive('mcmPreferences', ['$injector', '$rootScope', '$timeout', 'ConfigMapToolset', 'McmMapViewService',
		function($injector, $rootScope, $timeout, ConfigMapToolset, McmMapViewService){
		var GlobalEmitterServicesArray = $injector.get('GlobalEmitterServicesArray');
		return {
			restrict: 'AE',
			scope: {
			...
			controller: function ( $scope, $element ) {
				...
				$scope.configChanged = function(){
					var eventName = "mapStylingChangedEvent";
					GlobalEmitterServicesArray.get(eventName).broadcast('mcmPreferences');
				};
    		}
    	};
	}])

	.directive('mcmMapList', ['$injector', '$rootScope', '$timeout', 'ConfigMapToolset', 'KnalledgeMapVOsService', 'McmMapSchemaService', 'McmMapVariableOperatorService', 'McmMapVisualService',
		function($injector, $rootScope, $timeout, ConfigMapToolset, KnalledgeMapVOsService, McmMapSchemaService, McmMapVariableOperatorService, McmMapVisualService){
		// http://docs.angularjs.org/guide/directive
		var GlobalEmitterServicesArray = $injector.get('GlobalEmitterServicesArray');
		return {
			restrict: 'AE',
			...
			controller: function ( $scope, $element ) {
				...
				var mcmMapClientInterface = {
					...
					update: function(mapEntity){
						...
						GlobalEmitterServicesArray.get(eventName).broadcast('mcmMapList', mapEntity);
					},
					clicked: function(mapEntity){
						...
						var eventName = "mapEntitySelectedEvent";
						GlobalEmitterServicesArray.get(eventName).broadcast('mcmMapList', {mapEntity: mapEntity.vkNode, emittingScope: $scope});
					}
				};
				...
				$scope.goToParentClicked = function(){
					...
					if(parentEntities.length > 0){
						var eventName = "mapEntitySelectedEvent";
						GlobalEmitterServicesArray.get(eventName).broadcast('mcmMapList', {mapEntity: parentEntities[0], emittingScope: $scope});
					}
				};

				var eventName = "mapEntitySelectedEvent";
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMapList', function(eventObj) {
					if(eventObj.mapEntity){
						console.log("[mcmMapList.controller::GlobalEmitterServicesArray] ModelMap mapEntity (%s): %s", eventObj.mapEntity.kNode.type, eventObj.mapEntity.kNode.name);
				...
				eventName = "modelMapStructureChanged";
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMapList', function(mapEntity) {
					console.log("[mcmMapList.controller::GlobalEmitterServicesArray] modelMapStructureChanged");
					if(mapEntity){
						if(mapEntity.kNode){
							console.log("[mcmMapList.controller::GlobalEmitterServicesArray] ModelMap  mapEntity (%s): %s", mapEntity.kNode.type, mapEntity.kNode.name);
				...
				GlobalEmitterServicesArray.register(eventName);
				GlobalEmitterServicesArray.get(eventName).subscribe('mcmMapList', function(eventModel) {
					// not the map type we are interested in
					if(eventModel.properties.type != 'mcm_map') return;

					console.log("[mcmMapTools.controller::GlobalEmitterServicesArray] ModelMap  nodes(len: %d): %s",
						eventModel.map.nodes.length, JSON.stringify(eventModel.map.nodes));
					console.log("[mcmMapTools.controller::GlobalEmitterServicesArray] ModelMap  edges(len: %d): %s",
						eventModel.map.edges.length, JSON.stringify(eventModel.map.edges));
					...
					mcmMap.init(function(){
						...
						var eventName = "mapEntitySelectedEvent";
						GlobalEmitterServicesArray.get(eventName).broadcast('mcmMapList',
							{mapEntity: KnalledgeMapVOsService.mapStructure.rootNode, emittingScope: $scope});
					});
				});
    		}
    	};
