...
Map.prototype.init = function() {
	...
	// 	if(draggedIn){
	// 		that.client.mapEntityDraggedIn(d, dRelated);
	// 		that.update(that.model.nodes[0]);
	// 	}
	// };
...
// Map.prototype.clickNode = function(d, dom) {
	...
	// 	"unselected": true
	// });
	// this.model.map.nodes.forEach(function(d){d.isSelected = false;});
...
Map.prototype.placeModels = function(model){
	...
	this.update(this.model.map.nodes[0]);
};

// Map.prototype.getChildrenEdgeTypes = function(node){
// 	var children = {};
// 	for(var i in this.model.map.edges){
// 		var edge = this.model.map.edges[i];
// 		if(edge.sourceId == node.id){
// 			if(edge.type in children){
// 				children[edge.type] += 1;
// 			}else{
// 				children[edge.type] = 1;
// 			}
// 		}
...
// Map.prototype.getChildrenEdges = function(node, edgeType){
// 	var children = [];
// 	for(var i in this.model.map.edges){
// 		var edge = this.model.map.edges[i];
// 		if(edge.sourceId == node.id && ((typeof edgeType === 'undefined') || edge.type == edgeType)){
// 			children.push(edge);
// 		}
// 	}
// 	return children;
...
// Map.prototype.getChildrenNodes = function(node, edgeType){
// 	var children = [];
// 	for(var i in this.model.map.edges){
// 		var edge = this.model.map.edges[i];
// 		if(edge.sourceId == node.id && ((typeof edgeType === 'undefined') || edge.type == edgeType)){
// 			for(var j in this.model.map.nodes){
// 				var nodeChild = this.model.map.nodes[j];
// 				if(nodeChild.id == edge.targetId){
...
// Map.prototype.getMaxNodeId = function(){
// 	var maxId = -1;
// 	for(var i in this.model.map.nodes){
// 		if(this.model.map.nodes[i].id > maxId) maxId = this.model.map.nodes[i].id;
// 	}
// 	return maxId;
// };

// Map.prototype.getMaxEdgeId = function(){
// 	var id = -1;
// 	for(var i in this.model.map.edges){
// 		if(this.model.map.edges[i].id > id) id = this.model.map.edges[i].id;
// 	}	
// };
...
// 	this.model.map.nodes.push(nodeChild);
// 	this.model.map.edges.push(edge);
// };
...
// 	var that = this;
// 	this.processNodes(this.model.map.nodes);
// 	var nodeHtmlDatasets = this.updateHtml(this.model.map.nodes, source); // we need to update html nodes to calculate node heights in order to center them verticaly
...
// Map.prototype.updateHtml = function(nodesAll, source) {
// 	var that = this;
// 	if(!this.config.nodes.html.show) return;

// 	var nodes = [];
// 	for (var i in nodesAll){
// 		if(nodesAll[i].type in that.entityStyles && nodesAll[i].mapId == "1"){
...
// 		.html(function(d){
// 			return "" + that.getChildrenNodes(d, Map.CONTAINS_VARIABLE_IN).length;
// 		});

// 	// Output Variables
// 	nodeHtml.select(".var_out")
// 		.html(function(d){
// 			return "" + that.getChildrenNodes(d, Map.CONTAINS_VARIABLE_OUT).length;
// 		});
