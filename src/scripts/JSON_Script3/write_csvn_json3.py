...
#
#    % ./write_csvn_json3.py CSN_VarNames_v0.83.txt
#------------------------------------------------------
#
# Functions:
#    write_json()
...
def write_json( in_file='CSN_VarNames_v0.83.txt' ):
	...
	while (True):
		...
		if (READY):
			#------------------------------------------------------------
		    # Include parent object names, even if they didn't occur by
		    # themselves, because they have no quantities. (7/13/15)
		    #------------------------------------------------------------
			temp_name = object_name
			ancestor_list = list()
			while (True):
				index = temp_name.rfind('_')
				if (index == -1):
					break
				parent_name = temp_name[:index]
				if (parent_name not in object_name_list):
					ancestor_list.append( parent_name )
				temp_name = parent_name
			ancestor_list.reverse()
			for ancestor in ancestor_list:
				object_name_list.append( ancestor )
				out_unit.write( id_line_separator + id_line_prefix + ancestor + id_line_suffix + '\n')
				out_unit.write( prop_line_prefix1 + '""' + prop_line_suffix1 + '\n')
				out_unit.write( rdf_type_line + '\n')
				out_unit.write( pref_label_line_prefix + ancestor + pref_label_line_suffix + '\n')
